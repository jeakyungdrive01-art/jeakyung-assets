...
const SETTINGS = { show_in_sidebar: true, allow_comments: true, allow_replies: true, allow_attachments: false, allow_images: false, allow_anonymous: false, allow_notices: true, allow_important: true, show_views: true, allow_reactions: false, show_post_number: true, search_enabled: true, use_prefix: false, use_pinned: true, department_only: false, page_size: 20, default_sort: 'latest', max_file_size_mb: 20, max_inline_image_size_mb: 10, max_inline_images: 20, max_total_attachment_mb: 50, preserve_image_originals: false };
  ...
  return <section className="gw-admin-section" aria-labelledby="board-builder-title">
    ...
    <div className="gw-builder-layout"><div className="gw-compact-list" aria-label="게시판 목록">{catalog.boards.map((board) => <button type="button" key={board.id} onClick={() => selectBoard(board)}><strong>{board.name}</strong><span>{board.board_type} · /{board.slug}{board.archived_at ? ' · 보관' : ''}</span></button>)}</div>
      <form className="gw-builder-form" onSubmit={submit}><div className="gw-admin-form-grid">
        ...
        <label className="gw-field"><span>본문 이미지 1장 제한(MB)</span><input type="number" min="1" max="10" value={form.settings.max_inline_image_size_mb} onChange={(event) => updateSetting('max_inline_image_size_mb', Number(event.target.value))} /></label>
        <label className="gw-field"><span>본문 이미지 장수 제한</span><input type="number" min="1" max="20" value={form.settings.max_inline_images} onChange={(event) => updateSetting('max_inline_images', Number(event.target.value))} /></label>
        <label className="gw-field"><span>첨부 전체 제한(MB)</span><input type="number" min="1" max="50" value={form.settings.max_total_attachment_mb} onChange={(event) => updateSetting('max_total_attachment_mb', Number(event.target.value))} /></label>
